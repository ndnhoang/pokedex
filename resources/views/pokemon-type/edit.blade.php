...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <form class="form needs-validation custom-form" action="{{ route('type.edit', ['id' => $type->id]) }}" method="post" id="edit_pokemon_type_form">
                    @csrf
                    <div class="card-header">Edit Pokemon Type</div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" required id="name" value="{{ old('name') ? old('name') : $type->name }}" class="form-control">
                        </div>
                        <div class="form-group">
                                    ...
                                    <select id="type_0" name="type_0[]" class="custom-select select-multi" multiple placeholder="Select type">
                                        @if ($types)
                                            @foreach ($types as $type)
                                                ...
                                                @elseif ($type_0 && in_array($type->id, $type_0))
                                                    <option value="{{ $type->id }}" selected>{{ $type->name }}</option>
                                                @else
                                        ...
                                        @if ($types)
                                            @foreach ($types as $type)
                                                @if (old('type_50') && in_array($type->id, old('type_50')))
                                                    <option value="{{ $type->id }}" selected>{{ $type->name }}</option>
                                                @elseif ($type_50 && in_array($type->id, $type_50))
                                                    <option value="{{ $type->id }}" selected>{{ $type->name }}</option>
                                                @else
                                                    <option value="{{ $type->id }}">{{ $type->name }}</option>
                                                @endif
                                        ...
                                        @if ($types)
                                            @foreach ($types as $type)
                                                @if (old('type_200') && in_array($type->id, old('type_200')))
                                                    <option value="{{ $type->id }}" selected>{{ $type->name }}</option>
                                                @elseif ($type_200 && in_array($type->id, $type_200))
                                                    <option value="{{ $type->id }}" selected>{{ $type->name }}</option>
                                                @else
                                                    <option value="{{ $type->id }}">{{ $type->name }}</option>
                                                @endif
